...
class SensorValue<T>(
    ...
    companion object {
        ...
        val EMPTY_4D = SensorValue(value = listOf(0f, 0f, 0f, 0f, 0f))
        val EMPTY_2TYPE = SensorValue(value = Pair(Triple(0f, 0f, 0f), Triple(0f, 0f, 0f)))
    }
}
...
fun SensorEvent.to4DSensorValue(): SensorValue<List<Float>> = SensorValue(
    value = listOf(values[0], values[1], values[2], values[3], values[4]),
    timestamp = timestamp,
    accuracy = accuracy
)

fun SensorEvent.to2TypeValue(): SensorValue<Pair<Triple<Float, Float, Float>, Triple<Float, Float, Float>>>
 = SensorValue(
     value = Pair(Triple(values[0], values[1], values[2]), Triple(values[3], values[4], values[5])),
     timestamp = timestamp,
     accuracy = accuracy
 )
